...
```
## Error: 'var_Tresults1a.csv' does not exist in current working directory ('C:/Users/kevin/OneDrive/2016 Stanford Summer/GitHub/testthatChem/demo').
```
...
```
## Error in is.data.frame(y): object 'var_T' not found
```
...
```
## Error: 'var_Tresults2a.csv' does not exist in current working directory ('C:/Users/kevin/OneDrive/2016 Stanford Summer/GitHub/testthatChem/demo').
```
...
```
## Error in is.data.frame(y): object 'var_T2' not found
```
...
## 11 FC(F)(F)C(F)(F)C(F)(F)C(F)(F)C… 74883 C6H5F9O JCMNMO… Grou…      1 4:2 …    3.82    3.27  -0.550    2.03
...
```r
bbplot <- ggplot(data = resultsCombined, mapping = aes(x = log_KOA, y = log_KAW, color = Group, label = CmpdID))+ 
...
```r
b_plot <- ggplot(data = resultsCombined, mapping = aes(x = log_KOC, y = log_KAW, color = Group, label = CmpdID))+
  geom_point()+
